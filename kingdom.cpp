...
Kingdom::Kingdom(const std::string& name)
    ...
    mapX(0), mapY(0) { // Initialize mapX and mapY
...
void Kingdom::setMapPosition(int x, int y) {
    if (x < 0 || y < 0) {
        throw std::invalid_argument("Map coordinates cannot be negative");
    }
    mapX = x;
    mapY = y;
}

int Kingdom::getMapX() const {
    return mapX;
}

int Kingdom::getMapY() const {
    return mapY;
}
